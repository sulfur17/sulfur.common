...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	НастройкиВыгрузки = НастройкиВыгрузки();
	Настройки.Вставить("НастройкиВыгрузки", НастройкиВыгрузки);
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	НастройкиВыгрузки = Настройки["НастройкиВыгрузки"];
	
	Если НастройкиВыгрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПравилаВыгрузки = НастройкиВыгрузки.ПравилаВыгрузки;
	
	Для Каждого Группа Из Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы() Цикл
		
		НастройкиГруппы = ПравилаВыгрузки[Группа.Наименование];
		Если НастройкиГруппы = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого Элемент Из Группа.ПолучитьЭлементы() Цикл
			
			НастройкиЭлемента = НастройкиГруппы[Элемент.Наименование];
			
			Если НастройкиЭлемента = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Элемент.Включить = НастройкиЭлемента["Включить"];
			Элемент.ИспользоватьОтбор = НастройкиЭлемента["ИспользоватьОтбор"];
			
			Если Элемент.ИспользоватьОтбор Тогда
				
				ЗначениеОтбора = НастройкиЭлемента["ЗначениеОтбора"];
				
				ЭлементПостроителя = Элемент.НастройкиПостроителя.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ЭлементПостроителя.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка");
				Если ТипЗнч(ЗначениеОтбора) = Тип("СписокЗначений") Тогда
					ЭлементПостроителя.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
				Иначе
					ЭлементПостроителя.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				КонецЕсли;
				ЭлементПостроителя.ПравоеЗначение = ЗначениеОтбора;
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайлаСДиалогом(ОписаниеОповещения, АдресФайла,Нстр("ru = 'Файл обмена'"),, УникальныйИдентификатор);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайлаСДиалогом(ОписаниеОповещения, АдресФайлаОбменаВХранилище,Нстр("ru = 'Файл обмена'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайлаСДиалогом(ОписаниеОповещения, АдресФайлаПравилВХранилище,Нстр("ru = 'Файл правил обмена'"),, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ПрочитатьПравилаОбменаСохраненные(Команда)
	
	ИмяФайлаПравил = ПрошлоеИмяФайлаПравил();
	
	Если Не ЗначениеЗаполнено(ИмяФайлаПравил) Тогда
		ПрочитатьПравилаОбмена(Команда);
	Иначе
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПрочитатьПравилаОбменаЗавершение", ЭтотОбъект);
		НачатьПомещениеФайлаСДиалогом(ОписаниеОповещения, АдресФайлаПравилВХранилище,
			Нстр("ru = 'Файл правил обмена'"),, УникальныйИдентификатор, ИмяФайлаПравил);
...
Процедура ИзменитьРасположениеФайла()
	...
	Элементы.ПрочитатьПравилаОбменаСохраненные.Видимость = ЭтоКлиент;
...
&НаСервере
Процедура СохранитьПараметрыФормыСервер()
	НастройкиВыгрузки = НастройкиВыгрузки();
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиФормыУниверсальныйОбменДаннымиXML", "НастройкиВыгрузки", НастройкиВыгрузки);
КонецПроцедуры

&НаСервере
Функция НастройкиВыгрузки()
	Результат = Новый Структура;
	
	Группы = Новый Структура;
	Для Каждого СтрокаТаблицы Из Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы() Цикл
		
		СвойстваГруппы = Новый Соответствие;
		
		Для Каждого Элемент Из СтрокаТаблицы.ПолучитьЭлементы() Цикл
			СвойстваЭлемента = Новый Соответствие;
			
			СвойстваЭлемента.Вставить("Включить", Элемент.Включить);
			СвойстваЭлемента.Вставить("ИспользоватьОтбор", Элемент.ИспользоватьОтбор);
			
			Если Элемент.ИспользоватьОтбор Тогда
				
				Для каждого Настройка из Элемент.НастройкиПостроителя.Элементы Цикл
					СвойстваЭлемента.Вставить("ЗначениеОтбора", Настройка.ПравоеЗначение);
				КонецЦикла;
				
			КонецЕсли;
			
			СвойстваГруппы.Вставить(Элемент.Наименование, СвойстваЭлемента);
		КонецЦикла;
		
		Группы.Вставить(СтрокаТаблицы.Наименование, СвойстваГруппы);
		
	КонецЦикла;
	
	Результат.Вставить("ПравилаВыгрузки", Группы);
	
	Возврат Результат;
КонецФункции

&НаСервереБезКонтекста
Функция ПрошлоеИмяФайлаПравил()
	
	Результат = ХранилищеСистемныхНастроек.Загрузить("УниверсальныйОбменданнымиXML", "ИмяФайлаПравил");
	
	Возврат Результат;
КонецФункции

&НаСервере
Процедура СохранитьИмяФайлаПравил(ИмяФайлаПравил)
	
	ХранилищеСистемныхНастроек.Сохранить("УниверсальныйОбменданнымиXML", "ИмяФайлаПравил", ИмяФайлаПравил);
...
&НаКлиенте
Процедура НачатьПомещениеФайлаСДиалогом(ОписаниеОповещения, АдресФайла, ЗаголовокДиалога, НенужныйПараметр = Неопределено, УникальныйИдентификатор, ПутьКФайлу = "")
	
	ДопПараметры = Новый Структура("ОписаниеОповещения", ОписаниеОповещения);
	ОписаниеПродолжения = Новый ОписаниеОповещения("ПомещенияФайлаСДиалогомЗавершение", ЭтотОбъект, ДопПараметры);
	
	//ПараметрыДиалогаПомещенияФайлов = Новый ПараметрыДиалогаПомещенияФайлов;
	//ПараметрыДиалогаПомещенияФайлов.Заголовок = ЗаголовокДиалога;
	//ПараметрыДиалогаПомещенияФайлов.МножественныйВыбор = Ложь;
	//ПараметрыДиалогаПомещенияФайлов.Фильтр = "*.xml|*.xml";
	...
	НачатьПомещениеФайлаНаСервер(ОписаниеПродолжения, , , АдресФайла, ПутьКФайлу, УникальныйИдентификатор);
	
	СохранитьИмяФайлаПравил(ПутьКФайлу);
	
КонецПроцедуры

&НаКлиенте
Процедура ПомещенияФайлаСДиалогомЗавершение(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	
	Если ОписаниеПомещенногоФайла = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИмяПроцедуры = ДополнительныеПараметры.ОписаниеОповещения.ИмяПроцедуры;
	Результат = Не ОписаниеПомещенногоФайла.ПомещениеФайлаОтменено;
	Адрес = ОписаниеПомещенногоФайла.Адрес;
	ВыбранноеИмяФайла = ОписаниеПомещенногоФайла.СсылкаНаФайл.Имя;
	ДополнительныеПараметры = Неопределено;
	
	СтрокаВыполнения = СтрШаблон("%1(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры)", ИмяПроцедуры);
	Выполнить(СтрокаВыполнения);
	
КонецПроцедуры

&НаСервере
Функция ОтборыПоДоговорам()
	
	Результат = Новый Массив;
	
	СтрокаПравилВыгрузкиДоговоров = АнкетированиеКлиентСервер.НайтиСтрокуВДанныхФормыДерево(Объект.ТаблицаПравилВыгрузки, 
		"Справочник.ДоговорыКонтрагентов", "ИмяОбъектаДляЗапроса", Истина);
	
	Если СтрокаПравилВыгрузкиДоговоров = -1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	НастройкиПостроителя = Объект.ТаблицаПравилВыгрузки.НайтиПоИдентификатору(СтрокаПравилВыгрузкиДоговоров).НастройкиПостроителя;
	
	Для Каждого ЭлементыНастройки Из НастройкиПостроителя.Элементы Цикл
		
		Если ЭлементыНастройки.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
			Результат.Добавить(ЭлементыНастройки.ПравоеЗначение);
		ИначеЕсли ЭлементыНастройки.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке Тогда
			Результат.Добавить(ЭлементыНастройки.ПравоеЗначение);
		КонецЕсли;
