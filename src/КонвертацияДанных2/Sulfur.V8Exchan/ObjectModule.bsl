...
//  УзелРодитель   - xml-узел-родитель
//  Узел           - подчиняемый узел
//  ДобавитьОтступов - надо ли добавлять отступов чтобы узел стартовал с уровня родителя + 1
//
Процедура ДобавитьПодчиненный(УзелРодитель, Узел, ДобавитьОтступов = Истина) Экспорт
	...
	// Начало Sulfur
	ДобавитьОтступовГлубже = "";
	Если ДобавитьОтступов Тогда
		СимволыОтступа = УзелРодитель.Параметры.СимволыОтступа;
		ДобавитьОтступовГлубже = Новый Массив;
		Для Сч = 1 По УзелРодитель.КонтекстПространствИмен.Глубина Цикл
			ДобавитьОтступовГлубже.Добавить(СимволыОтступа);
		КонецЦикла;
		ДобавитьОтступовГлубже = СтрСоединить(ДобавитьОтступовГлубже);
	КонецЕсли;
	
	ДобавитьПС = ДобавитьОтступов;
	
	ДоработатьИнформациюДляЗаписиВФайл(ИнформацияДляЗаписиВФайл, ДобавитьОтступовГлубже, ДобавитьПС);
	// Конец Sulfur
...
Процедура ДоработатьИнформациюДляЗаписиВФайл(ИнформацияДляЗаписиВФайл, Отступ = "", ДобавитьПС = Ложь)
	
	Если ДобавитьПС Тогда
		ИнформацияДляЗаписиВФайл = Символы.ПС + ИнформацияДляЗаписиВФайл;
	КонецЕсли;
	
	ДобавитьОтступ = (Отступ <> "");
	
	Строки = СтрРазделить(ИнформацияДляЗаписиВФайл, Символы.ПС);
	
	// Сдвигаем закрытие ссылки
	Закрытие = "</Ссылка>";
	ПозицияСсылки = Строки.Найти(Закрытие);
	СдвинутьСсылку = (ПозицияСсылки <> Неопределено И ПозицияСсылки < Строки.Количество() - 1);
	Если СдвинутьСсылку Тогда
		СледующаяПозиция = ПозицияСсылки + 1;
		Строки.Установить(ПозицияСсылки, Закрытие + Строки[СледующаяПозиция]);
		Строки.Удалить(СледующаяПозиция);
	КонецЕсли;
	
	Если ДобавитьОтступ
		Или СдвинутьСсылку Тогда
		
		ИнформацияДляЗаписиВФайл = Отступ + СтрСоединить(Строки, Символы.ПС + Отступ); // Добавляем отступ
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьОтступПередЗакрытиемСсылки(ИнформацияДляЗаписиВФайл, Отступ)
	...
	Закрытие = "</Ссылка>";
	ПозицияСсылки = Строки.Найти(Закрытие);
	СдвинутьСсылку = (ПозицияСсылки <> Неопределено И ПозицияСсылки = Строки.ВГраница());
	Если СдвинутьСсылку Тогда
		Строки.Установить(ПозицияСсылки, Отступ + Строки[ПозицияСсылки]);
	КонецЕсли;
	
	Если СдвинутьСсылку Тогда
		ИнформацияДляЗаписиВФайл = СтрСоединить(Строки, Символы.ПС);
	КонецЕсли;
...
Функция СоздатьНовыйОбъект(Тип, СвойстваПоиска, Объект = Неопределено, 
	...
	ИначеЕсли ИмяТипа = "РегистрНакопления" Тогда
		...
		ВызватьИсключение "РегистрНакопления недоступен в КД";
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				ЭтоСсылочныйТипОбъекта = НЕ( ИмяТипаОбъекта = "РегистрСведений"
					ИЛИ ИмяТипаОбъекта = "РегистрНакопления"
				...
				Если НужноЗаписатьОбъект Тогда
					...
					// Иванов
					Если ТипЗнч(Объект) = Тип("КонстантыНабор") Тогда
						// Чтобы записывать только нужные константы
						СтарыйОбъект = Объект;
						НаборКонстантСтрокой = СтрСоединить(НаборКонстант, ",");
						Объект = Константы.СоздатьНабор(НаборКонстантСтрокой);
						ЗаполнитьЗначенияСвойств(Объект, СтарыйОбъект, НаборКонстантСтрокой);
					КонецЕсли;
...
Функция ПолучитьНаборДвиженийДокумента(СсылкаНаДокумент, ВидИсточника, ИмяРегистра)
	...
	Если Не Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СсылкаНаДокумент)) Тогда
		ВызватьИсключение "СсылкаНаДокумент не может быть регистратором набора движений";
	КонецЕсли;
...
Процедура ПроизвестиЗаписьДанныхВГоловнойУзел(УзелКоллекцииСвойств, СтруктураУзлаСвойств, УзелСвойства)
	...
	Иначе
		ДобавитьПодчиненный(УзелКоллекцииСвойств, УзелСвойства, Истина);
	КонецЕсли;
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если НеНужноВыгружатьПоСоответствиюЗначений Тогда
		...
		Если ПКО.СвойстваПоиска.Количество() > 0 
			...
			Отступ = УзелСсылки.Параметры.СимволыОтступа;
			УзелСсылки = УзелСсылки.Закрыть();
			
			//ДобавитьОтступПередЗакрытиемСсылки(УзелСсылки, Отступ);
...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено, Свойства = Неопределено) Экспорт
